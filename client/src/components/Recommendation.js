...
const Reccomendation = () => {
  ...
  return (
    <div style={{
      ...
      <Typography.Title level={3}>We selected 10 movies base on your preferences, and their ratings are all higher than the average ratings in each genre. Enjoy!</Typography.Title>

      <Row gutter={12} style={{ marginTop: 16, display: 'flex', justifyContent: 'center' }}>
        {movies.map((movie) => (
          <Col span={4} key={movie.title} style={{ marginLeft: `20px` }}>
            <Card
              hoverable
